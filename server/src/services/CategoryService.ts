...
export class CategoryService extends BaseService {
  ...
  async getCategories() {
    ...
    const categories = await this.safeQuery(sql);
    return categories;
  }
  async getProductsByCategory(pagination: Pagination) {
  ...
  async createCategory(category: ProductCategoryTree) {
  ...
  async updateCategory(category: ProductCategoryTree) {
  ...
  async deleteCategory(id: number) {
    ...
    const category = await this.safeQuery(sql, [id]);
    return category;
  }
}
