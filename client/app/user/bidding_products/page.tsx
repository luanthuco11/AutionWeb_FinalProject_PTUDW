...
import BiddingProduct from "@/components/BiddingProduct"
...
for (let i = 6; i <= 30; i++) {
  mockProducts.push({
    ...
    current_price: `${11_000_000 + i * 100_000} `,
  });
}

const BiddingProductPage = () => {
    const [biddingProduct, setBiddingProduct] = useState<ProductType.BiddingProduct[]>(mockProducts)
        ...
        <div className='bg-card shadow-sm rounded-lg p-8'>
            ...
            <div className="flex flex-col gap-5">
                {biddingProduct.map((bP, index) => (
                    <BiddingProduct key={index} product={bP}/>
                ))}
            </div>
        </div>
    )
